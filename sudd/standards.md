...
- opencode: `opencode run -m "{provider}/{model}" --format json --dir "{project_dir}" "{prompt}"`
- crush: `crush run -m "{provider}/{model}" -y "{prompt}"`
- claude-code: `claude -p --bare --model {model} --dangerously-skip-permissions --max-turns {max_turns} "{prompt}"`

Note: `--bare` is correct for claude-code subprocesses (they need file tools, not skills). The orchestrator watchdog must NOT use `--bare`.
